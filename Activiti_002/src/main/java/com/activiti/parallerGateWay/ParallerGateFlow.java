package com.activiti.parallerGateWay;
...
public class ParallerGateFlow {
	...
	public void processDefined() {
		...
		InputStream inputStreambpmn = this.getClass().getResourceAsStream("parallerGateWay.bpmn");
		InputStream inputStreampng = this.getClass().getResourceAsStream("parallerGateWay.png");
		...
		Deployment deployment = processEngine.getRepositoryService()// 创建流程仓库，放入流程定义,他会默认加载avtiviti.cfg.xml
				...
				.name("parallerGateWay")// 定义部署名
				.addInputStream("parallerGateWay.bpmn", inputStreambpmn).addInputStream("parallerGateWay.png", inputStreampng)
				.deploy();
